...
export type ExistingWorkflowForPrompt = {
  nodes?: Array<{ id: string; data?: { label?: string } }>;
  edges?: Array<{ id: string; source: string; target: string }>;
  name?: string;
};

function encodeMessage(encoder: TextEncoder, message: WorkflowStreamMessage) {
...
export function parseWorkflowOperationLine(
  line: string
  ...
  try {
    return JSON.parse(trimmed) as WorkflowOperation;
  } catch {
    return null;
  }
}
...
function tryParseAndEnqueueOperation(
  ...
  const operation = parseWorkflowOperationLine(line);
  if (!operation) {
...
async function processOperationStream(
  ...
  tryParseAndEnqueueOperation(buffer, encoder, controller, operationCount);
...
export function createWorkflowOperationStream(
  textStream: AsyncIterable<string>
  ...
  return new ReadableStream({
    async start(controller) {
      try {
        await processOperationStream(textStream, encoder, controller);
        controller.close();
      } catch (error) {
        controller.enqueue(
          encodeMessage(encoder, {
...
export function buildWorkflowSystemPrompt(pluginActionPrompts: string): string {
...
export function buildWorkflowUserPrompt(options: {
  prompt: string;
  existingWorkflow?: ExistingWorkflowForPrompt;
}) {
  const { prompt, existingWorkflow } = options;
